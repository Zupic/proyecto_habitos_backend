...
router.post('/register',async function (req,res,next){
  try{
    ...
    //generamos  un salt y hash con bcrypt, encripta
...
router.post('/login', async function (req, res, next){
  try{
    ...
    if (!user) return res.status(400).json ({error:"Usuario no enconteado"});

    //Comparar la contrasela ingresada con el hash almacenado 
    const isMatch = await bcrypt.compare(password, user.password);
    if(!isMatch) return (400).json({error: "Contraseña incorrecta"});
    ...
    res.cookie('habitToken',token,{
      ...
      maxAge: 7 * (24) * 60 *60 *100// 7 dás de duración
   });
